  ...
  <div class="helplist_box">
    <ul>
      <li v-for="(item,index) in list">
        <div class="row">
            <router-link :to="'/infodetail?id='+item.Id+'&ver='+ver" v-if="item.Title.length>17">
              {{item.Title.substr(0,17)}}...
            </router-link>
            <router-link :to="'/infodetail?id='+item.Id+'&ver='+ver" v-else>
              {{item.Title}}
            </router-link>
        </div>
      </li>
...
export default {
  ...
  created:function(){
     ...
     this.$http(this.$api.InfoList({"CateId":q.CateId}))
    .then((data) => {
